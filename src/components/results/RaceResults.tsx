...
const RaceResults: React.FC<{season?: string, round?: string}> = ({season, round}) => {
  ...
  return (
    <IonList lines="full">
      ...
      {results.map(result =>
        <IonItem key={result.position} button routerLink={`/racedata/${season}/${round}/${result.Driver.driverId}`} className={result.FastestLap?.rank === '1' ? 'fastest-lap' : undefined}>
          ...
          <IonBadge color="medium" mode="ios" className="race-time">{result.Time ? result.Time.time : (result.status === '+1 Lap' ? result.status : 'DNF')}</IonBadge>
